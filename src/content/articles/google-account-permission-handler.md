...
description: "Google Forms や Drive の権限エラーを、学内ヘルプセンターに組み込める SolidJS フロントエンドと PHP API で解決した経緯と運用メモ"
pubDate: "2025-11-16"
emoji: "🔐"
tags: ["google", "php", "api", "solidjs", "googleforms"]
---

## きっかけ

Google Forms や Drive で権限エラーが出たときにアカウント切替を簡単にするため、個人で [google-account-permission-handler](https://github.com/minagishl/google-account-permission-handler) を公開し、学内向けに運用した経緯を書き残します  
...
在籍していたデジタル委員会 ナレッジベース部門（旧ナレッジベース実行委員）で既存の学内ヘルプセンターに組み込むことになり、まず個人で公開していたフロントエンド版を学内で案内し、次にバックエンドを PHP API に書き換えました  
ヘルプセンターに iframe で資料を埋め込む際に表示されない問題や、フォームにアクセスできない問題をこの API で解消し、学内ポータルからシームレスに使えるようにしました  
PHP 版は学内専用として非公開運用に切り替え、CORS 設定やトークン検証フィルターを追加し、ログを Cloud Logging に集約するなど運用面も整備しました
